...
**方式一（推荐）**：去 [Releases](https://github.com/fulanduolu138/days-together/releases/latest) 页面，在 Assets 里点 APK 下载——那里是正经的下载按钮。

**方式二**：直接在仓库里打开 [`相爱-2.0.0.apk`](相爱-2.0.0.apk)，然后点页面**右上角的下载图标（Download raw file）**保存。（GitHub 的文件页只是"查看"，不会自动下载，很多人会卡在这一步。）

## 安装

1. 下载后点开安装；系统会提示"未知来源 / Play 保护"，允许这一次就好；
2. 如果装过更早的内部测试版，先卸载再装（签名不同，覆盖不了）；
